...
# Add safe globals for YOLO model classes
...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    is_coco = isinstance(data_dict.get('val'), str) and data_dict['val'].endswith(f'val{os.sep}data')  # COCO dataset
    ...
    if pretrained:
        ...
        # MODIFIED: Add weights_only=False to avoid PyTorch 2.6+ security restriction
    ...
    # rest of the train function remains unchanged
...
def parse_opt(known=False):
    ...
    parser.add_argument('--flat-cos-lr', action='store_true', help='flat+cos LR scheduler')
    parser.add_argument('--fixed-lr', action='store_true', help='fixed LR at --lr0')
    ...
    parser.add_argument('--min-items', type=int, default=0, help='Minimum number of images in a batch')
    parser.add_argument('--close-mosaic', type=int, default=0, help='Close mosaic augmentation for last n epochs')
...
def main(opt, callbacks=Callbacks()):
    ...
    if RANK in {-1, 0}:
        print_args(vars(opt))
        # Bỏ qua check_git_status() vì không cần thiết
        # check_requirements()
